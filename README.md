...
- 시청자는 "재미있거나 유용한 콘텐츠"를 보고 있다고 느껴야 합니다.
- 제품은 이야기의 "자연스러운 해결책"으로 등장해야 합니다.
- 구매 유도 문구("지금 구매", "할인", "링크" 등)는 영상 스크립트에 절대 포함하지 않습니다.

[3막 구조 — 반드시 준수]
1막 훅 (0~3초): 제품명 없이 감정/상황/궁금증으로 시작. 시청자가 멈추게 만드는 것이 목적.
2막 몸통 (3~20초): 공감대 형성 → 정보/이야기 전개 → 제품이 해결책으로 자연 등장.
3막 클로징 (마지막 2~3초): 짧고 감정이 담긴 한 마디로 마무리. CTA 없음.
...
[태그 체계 — visual_tag에 반드시 아래 목록 중에서만 선택]
  ...
  "segments": [
    {
      "time": "00~03s",
      "narration": "나레이션 텍스트",
      "on_screen_text": "화면에 표시할 자막 (핵심어만, 10자 이내)",
      "visual_tag": ["태그1", "태그2", "태그3"],
      "sfx": "효과음 유형 (whoosh/ding/boing/없음 중 하나)"
    }
  ],
  "closing_line": "마지막 한 마디 (10자 이내, 감정 담긴 짧은 문장)",
...
### 7-3. today_script.json 저장

1. Claude.ai가 출력한 JSON 전체 복사
2. 메모장 열기 → 붙여넣기
3. `파일 → 다른 이름으로 저장`
4. 파일 형식: **모든 파일** 선택
5. 파일명: `today_script.json`
6. 저장 위치: `D:\WISSLIST\scripts_json\`
